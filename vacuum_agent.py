...
class ContinuumWorld:
    ...
    Arguments:
        ...
        dirt - a list of lists representing the entire world with the dirt value
            of each tile
    '''
    ...
    '''
    This function will change the position of the agent when movement actions
    are performed, and will change the tile to clean on suck action
    ...
    def perform_action(self, agent, action):
        ...
        if action=='D':
            agent.position[0] += 1
        if action=='S':
            agent.score += self.dirt[agent.position[0]][agent.position[1]]
            self.dirt[agent.position[0]][agent.position[1]] = 0

    '''
    This function prints the current world representation with dirt in each tile
    ...
    def print_dirt(self, agent, position):
        ...
        print("["+str(self.dirt[agent.position[0]][agent.position[1]])+"]",end=" ")
...
class SimpleReflexAgent:
    ...
    def clean_grid(self):
        ...
        for i in range(self.max_moves):
            ...
            self.world.perform_action(self, step)
...
'''
This function reads the 'environ.txt' file to retrieve initital state of world
and agent.
